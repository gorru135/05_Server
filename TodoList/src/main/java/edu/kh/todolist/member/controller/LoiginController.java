...
@WebServlet("/login")
public class LoiginController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(member != null) { // 로그인 성공
				...
				// 			 이를 이용해서 응답 
				//			 위임 시 request , responese 객체를 함께 위임 
				
				// redirect : 요청 처리 후 자체적인 화면이 없어서 
				// 			  화면이 있는 다른 요청을 다시 호출(재요청)
				// 			  request , responese 폐기됨 -> 재요청 후 응답받은 페이지에서 사용 불가 
				...
				// 이전 페이지로 redirect (request , responese 폐기됨);
